...
# 0 = unlimited recs (Koyeb-safe via fetch pool + live part upload)
MAX_CONCURRENT_REC = max(0, _int("SC_MAX_REC", 0))
MAX_REC_PER_USER = max(0, _int("SC_MAX_REC_PER_USER", 0))
MAX_MONITORS = max(4, min(200, _int("SC_MAX_MONITORS", 80)))
MONITOR_POLL = max(1.0, float(_str("SC_MONITOR_POLL", "1.2") or "1.2"))
MONITOR_COOLDOWN = max(1, _int("SC_MONITOR_COOLDOWN", 2))
PING_PER_TICK = max(1, min(12, _int("SC_PING_PER_TICK", 6)))
...
WORKERS = max(1, _int("SC_WORKERS", 8))
FETCH_CONCURRENCY = max(4, min(32, _int("SC_FETCH_CONCURRENCY", 16)))
DISK_MIN_MB = max(80, _int("SC_DISK_MIN_MB", 220))
PORT = _int("PORT", 8080)
